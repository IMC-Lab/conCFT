...
%% Initialize task %%
    ...
    fprintf(datafilepointer, ...
        ...
        'fixOnsetTime', 'jitter1', 'jitter2', 'jitter3', 'jitterStim', 'jitterResp', 'jitterAccuracy',... 
...
%% start trials %%
    for trial = 1:runLength
       ...
       % When the participant clicks a button (started imagining target CFT) then record the response time. Can easily make it so only the first response is recorded by adding a conditional.
        ...
        fprintf(datafilepointer, '\n%i,%i,%i,%i,%f,%f,%f,%f,%s,%s,%f,%f,%i,%s,%f,%f,%f,%f,%s,%f,%s,%f,%f,%s,%f,%f,%s,%f,%f', ...
                subID, run, trial, condList(trial),...
                fixOnsetTime, jitter1(trial), jitter2(trial), jitter3(trial), string(jitterStim), mat2str(jitterResp), jitterAccuracy, ...
