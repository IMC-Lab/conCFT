...
library(lmerTest) #for viewing lme4 results
library(emmeans) #to extract model estimates for plotting
library(ltm) #to compute cronbachs alpha
library(sjPlot) #to make pretty lmer summary tables
library(patchwork) #for combining multiple plots into one figure
library(paletti) #for making a custom color palette
library(tidyverse) #for easy data wrangling & beautiful plot-making 
...
session1_data <- session1_data %>%
  ...
  mutate(condition=factor(condition, levels=c(0,1,2), labels=c('Practice', 'Dispositional', 'Situational'))) %>%
...
session2_data <- session2_data %>%
  rename(subID = sub) %>%
  mutate(condition = factor(condition, levels=c(1,2), labels=c('Dispositional', 'Situational')))
...
psq_data <- psq_data %>%
  rename(subID = sub,
         condition = Condition,
         eventTitle = Title) %>%
  mutate(condition=factor(condition, levels=c('Self','Context'), labels=c('Dispositional', 'Situational')))
...
session2_plot <- function(aes_DV, DV, title, n) {
  ggplot(all_data, aes_(x=aes_DV, fill=~condition)) +
    ...
    geom_errorbarh(data=filter(all_data, condition=='Dispositional'), aes(xmin=mean({{DV}},na.rm=T)-sd({{DV}},na.rm=T), xmax=mean({{DV}}, na.rm=T)+sd({{DV}}, na.rm=T), y=0, height=50), size=1) +
    geom_errorbarh(data=filter(all_data, condition=='Situational'),  aes(xmin=mean({{DV}},na.rm=T)-sd({{DV}},na.rm=T), xmax=mean({{DV}}, na.rm=T)+sd({{DV}}, na.rm=T), y=0, height=50), size=1) +
    geom_point(data=filter(all_data, condition=='Dispositional'), aes(x=mean({{DV}},na.rm=T), y=0), size=3) +
    geom_point(data=filter(all_data, condition=='Situational'), aes(x=mean({{DV}},na.rm=T), y=0), size=3) +
    geom_vline(data=filter(all_data, condition=='Dispositional'), aes(xintercept=mean({{DV}},na.rm=T)), size=0.75, linetype='dashed', color='gray30') +
    geom_vline(data=filter(all_data, condition=='Situational'), aes(xintercept=mean({{DV}},na.rm=T)), size=0.75, linetype='dashed', color='gray30') +
...
ggplot(all_data, aes(x=controlResponse, fill=condition)) +
    facet_wrap(vars(condition)) +
    geom_bar(color='black', width=1, alpha=0.7) +
    geom_errorbarh(data=filter(all_data, condition=='Dispositional'), aes(xmin=mean(controlResponse, na.rm=T)-sd(controlResponse, na.rm=T), xmax=mean(controlResponse,na.rm=T)+sd(controlResponse, na.rm=T), y=0, height=50), size=1) +
    geom_errorbarh(data=filter(all_data, condition=='Situational'), aes(xmin=mean(controlResponse, na.rm=T)-sd(controlResponse, na.rm=T), xmax=mean(controlResponse,na.rm=T)+sd(controlResponse, na.rm=T), y=0, height=50), size=1)
    geom_point(data=filter(summary_stat, condition=='Dispositional'), aes(x=controlMean, y=0), size=3) +
    geom_point(data=filter(summary_stat, condition=='Situational'), aes(x=controlMean, y=0), size=3) +
    geom_vline(data=filter(summary_stat, condition=='Dispositional'), aes(xintercept=controlMean), size=0.75, linetype='dashed', color='gray30') +
    geom_vline(data=filter(summary_stat, condition=='Situational'), aes(xintercept=controlMean), size=0.75, linetype='dashed', color='gray30') +
    scale_x_continuous(breaks=1:n) +
    custom_fill() +
    labs(y='Count', x='Rating', title=title) +
    theme_light() +
...
tab_model(controlModel,
          ...
          pred.labels=c('Intercept', 'Condition (situational)'))
